...
# # Set the random seed for reproducibility in PyTorch, Python, and NumPy
...
# # Use the seed in directory names or filenames if you want to isolate each run.
# data_root = f'./data_{random_seed}'
...
# # Load the CIFAR-100 dataset using the custom directory
# train_dataset = datasets.CIFAR100(root=data_root, train=True, transform=transform, download=True)
# train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=4)

# val_dataset = datasets.CIFAR100(root=data_root, train=False, transform=transform, download=True)
# val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False, num_workers=4)
...
# def train_main_model(model, device, train_loader, optimizer, criterion, epochs=25):
#     model.train()
#     for epoch in range(epochs):
#         running_loss = 0.0
#         correct = 0
...
# # Auxiliary model training function (for a given number of epochs)
# def train_auxiliary_models(main_model, auxiliary_models, device, train_loader, optimizers, criteria, epochs=5):
#     for param in main_model.parameters():
#         param.requires_grad = False  # Freeze main model weights

#     main_model.eval()  # Main model in eval mode
...
# criterion = nn.CrossEntropyLoss()
# optimizer = optim.Adam(main_model.fc.parameters(), lr=0.001)

# train_main_model(main_model, device, train_loader, optimizer, criterion, epochs=25)

# print("Training auxiliary models in 5-epoch increments...")
# total_aux_epochs = 25
# increment = 5
...
#     print(f"Evaluating auxiliary models after {i+increment} epochs:")
#     evaluate_models(main_model, auxiliary_models, device, val_loader)

# # Example of saving the main model checkpoint with the seed appended
# checkpoint_name = f'checkpoint_main_{random_seed}.pth'
# torch.save(main_model.state_dict(), checkpoint_name)

# # Similarly, you could save auxiliary model checkpoints
# for layer_name, aux_model in auxiliary_models.items():
#     aux_checkpoint_name = f'checkpoint_{layer_name}_{random_seed}.pth'
#     torch.save(aux_model.state_dict(), aux_checkpoint_name)
import os
import random
import torch
import torch.nn as nn
...
# Load pre-trained ResNet18 model on ImageNet
main_model = models.resnet18(pretrained=True).to(device)
...
def train_main_model(model, device, train_loader, optimizer, criterion, epochs=25):
    ...
    for epoch in range(epochs):
        ...
        accuracy = 100 * correct / total
        print(f"Main Model - Epoch {epoch + 1}, Loss: {running_loss / len(train_loader):.4f}, Training Accuracy: {accuracy:.2f}%")
...
for i in range(0, total_aux_epochs, increment):
    train_auxiliary_models(main_model, auxiliary_models, device, train_loader, aux_optimizers, aux_criteria, epochs=increment)
    print(f"Evaluating auxiliary models after {i+increment} epochs:", flush=True)
    evaluate_models(main_model, auxiliary_models, device, val_loader)

# Save the main model and auxiliary models with the seed in their filenames
checkpoint_name = f'checkpoint_main_{random_seed}.pth'
torch.save(main_model.state_dict(), checkpoint_name)
